...
func findSecondMinimumValue(root *TreeNode) int {
	minVal := root.Val
	secondMinVal := math.MaxInt64

	traverse(root, &minVal, &secondMinVal)
	if secondMinVal == math.MaxInt64 {
		return -1
	}
	return secondMinVal
}

func traverse(root *TreeNode, m *int, s *int)  {
	if root == nil {
		return
	}
	if *m < root.Val && root.Val < *s {
		*s = root.Val
	} else if *m == root.Val {
		traverse(root.Left, m, s)
		traverse(root.Right, m, s)
	}
